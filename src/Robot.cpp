...
void Robot::Run()
{
	if (!StopRobot()) // Robot in azione
	{
		...
		if (BlackTile() && NotInRamp())
		{
			...
			while (cs->c_comp <= MAX_VALUE_BLACK_TILE)
			{
				cs->getData();
			}
		...
		if (digitalReadFast(R_COLLISION_SX_PIN) && (imu->y >= -10 && imu->y <= 10))
		{
		...
		else if (digitalReadFast(R_COLLISION_DX_PIN) && (imu->y >= -10 && imu->y <= 10))
		{
		...
		if ((NewTile() && NotInRamp()) || (GetFrontDistance() <= 50 && lasers->sensors[VL53L5CX::FW]->GetData()->target_status[DISTANCE_SENSOR_CELL_FRONT] == 5))
		{
		...
		{
			int16_t power_to_add = 0;
			if (imu->y > 0)
			{
				power_to_add = imu->y / 1.2;
			}
...
void Robot::DecideTurn(bool left_blocked, bool front_blocked, bool right_blocked, bool tile_already_visited, bool blue_tile)
{
	...
	if (right_blocked && left_blocked && front_blocked)
	{
		...
		if (path_calculated && !path_to_tile.empty())
		{
...
void Robot::SetNewTileDistances()
{
	...
	UpdateSensorNumBlocking(VL53L5CX::BW);
	UpdateSensorNumBlocking(VL53L5CX::FW);
	if ((GetBackDistance() > 1200 || lasers->sensors[VL53L5CX::BW]->GetData()->target_status[DISTANCE_SENSOR_CELL_BACK] != 5) && (GetFrontDistance() > 1200 || lasers->sensors[VL53L5CX::FW]->GetData()->target_status[DISTANCE_SENSOR_CELL_FRONT] != 5))
	{
	...
	if (GetBackDistance() > 1200 || lasers->sensors[VL53L5CX::BW]->GetData()->target_status[DISTANCE_SENSOR_CELL_BACK] != 5)
	{
...
{
	return cs->c_comp <= MAX_VALUE_BLACK_TILE;
}

bool Robot::BlueTile()
{
	return cs->c_comp <= MAX_VALUE_BLUE_TILE;
}
...
bool Robot::NewTileGoingBack()
{
	...
	{
		UpdateGyroBlocking();
		if (((GetBackDistance() <= back_distance_to_reach && lasers->sensors[VL53L5CX::BW]->GetData()->target_status[DISTANCE_SENSOR_CELL_BACK] == 5) || (GetFrontDistance() >= front_distance_to_reach && lasers->sensors[VL53L5CX::FW]->GetData()->target_status[DISTANCE_SENSOR_CELL_FRONT] == 5)))
		{
			Serial.println("---NewTile---: ");
			Serial.print("Posteriore: ");
			Serial.println(GetBackDistance());
...
void Robot::Turn(int16_t degree)
{
	...
	if (degree > 0) // Giro a destra
	{
		...
		while (imu->z <= desired_angle - ADDITIONAL_ANGLE_TO_OVERCOME)
		{
			UpdateGyroBlocking();
			ms->SetPower(-TURN_SPEED, TURN_SPEED);
			if (millis() - time_started_turning > MAX_TIME_TURNING)
			{
				ms->SetPower(TURN_SPEED, -TURN_SPEED);
				FakeDelay(350);
				time_started_turning = millis();
